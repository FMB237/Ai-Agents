#This is mainly the script which will permit me to store data in FAISS data
import requests
...
vector_store = {} #To store the vector
...
#Functions to retrieve chunks and answer questions
def retrieve_and_answer(query):
    global index,vector_store

    #Convert user query into an embedded
    query_vector = np.array(embeddings.embed_query(query), dtype=np.float32).reshape(1, -1) 

    #Search FAISS
    D,I= index.search(query_vector,k=2)  #Used to retrive to similaar chunks
